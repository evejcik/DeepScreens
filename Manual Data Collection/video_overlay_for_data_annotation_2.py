...
def draw_bbox_and_label(img, instance, instance_ind, label):
    ...
    color = (255,0,0)
    cv2.rectangle(img, (x1, y1+141), (x2, y2+141), color, 3)
    ...
    while True:
        ...
        for instance_ind, instance in enumerate(instances):
            ...
            if joint is not None and show_joint_bbox:
                joint_id = meta['keypoint_name2id'].get(joint)
                if (joint_id is not None
                        and frame_id in joints_map
                        and instance_ind in joints_map[frame_id]):
                    pt = joints_map[frame_id][instance_ind].get(joint_id)
                    if pt is not None:
                        x, y = int(pt['x']), int(pt['y'])
                        draw_joint_bbox(frame, x, y, color=color)
